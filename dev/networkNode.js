...
let privateKey; // private key of server to get reward when mining
let publicKey; // public key of server to get reward when mining
let backup; // the main blockchain
let nodes = []; // connected node
...
async function initial() {
    ...
    if (port === '3000') {
        ...
        privateKey = uuid().split('-').join(''); //privateKey
        publicKey = sha256(privateKey); //publicKey
        const master = backup.createNewTransaction(1000000, "system-reward", publicKey);
        ...
        try {
            const socket = require('socket.io-client')('ws://172.0.0.1:3000');
            ...
            socket.on('PT', async (pt) => {
                console.log('newTranscation: ', pt);
            })

            const data = await axios.get('http://localhost:3000/blockchain')
            backup = Object.setPrototypeOf(data.data, Blockchain.prototype);
...
io.on('connection', async (socket) => {
    ...
    socket.emit('PT', backup.pendingTransactions);
    ...
    */
    app.post('/transaction/broadcast', async (req, res) => {
        ...
        if (port != 3000) {
            const data = await axios.post("http://localhost:3000/transaction/broadcast", req.body);
        }

        console.log('broadcast')
        const amount = parseFloat(req.body.amount);
        const newTransaction = nodes[nodes.length - 1].createNewTransaction(amount, req.body.sender, req.body.recipient);
        console.log('broadcast newTx: ', newTransaction)
        let flag = true;
        ...
        if ((sender !== "system-reward") && (sender !== "system-reward: new user") && (sender !== "system-reward: invitation confirmed")) {
            ...
            console.log("privateKey isValid", privateKey_Is_Valid)
            ...
            console.log('addressData: ', addressData1)
...
*/
    ...
    */
    app.post('/hashKeys', (req, res) => {
        ...
        const privateKey_Is_Valid = sha256(privateKey) === publicKey;

        const addressData = backup.getAddressData(publicKey);
